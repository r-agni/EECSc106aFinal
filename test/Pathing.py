    ...
    try:
        ...
        while True:
            ...
            if demo is not None:
                ...
                vx = raw_vx * vel_scale
                vy = raw_vy * vel_scale
                # World frame (assuming yaw ≈ 0): +x = right, +y = forward
                v_forward = -vx          # forward
                v_right = vy            # right (assuming +vy is right in your setup)
                ...
                # NEW: read yaw if available (for Δθ only; does not affect x,y integration)
                if hasattr(demo, "psi"):
                    # Typical AR.Drone: psi in millidegrees
                    yaw_deg = float(demo.psi) / 1000.0
                elif hasattr(demo, "yaw"):
                    yaw_deg = float(demo.yaw)
                else:
                    yaw_deg = 0.0
                est_theta = np.deg2rad(yaw_deg)

                # Integrate + scale (same as before)
                est_x_r += v_right * dt
                est_y_r += v_forward * dt
                est_x = SCALE_X * est_x_r
                est_y = SCALE_Y * est_y_r
            ...
            path_x.append(est_x)
            path_y.append(est_y)
            path_theta.append(est_theta)  # NEW
        ...
        #-------------------------
        # Start is path_x[0], path_y[0]; end is est_x, est_y
        delta_x = est_x - path_x[0]
        ...
        plt.figure()
        plt.plot(path_x, path_y, marker='o')
        plt.scatter([path_x[0]], [path_y[0]], s=80)          # start
        plt.scatter([est_x], [est_y], s=80)                  # end
        plt.text(path_x[0], path_y[0], " Start (0,0)", va='bottom', ha='left')
        plt.text(est_x, est_y, " End", va='bottom', ha='left')
        plt.xlabel("x (m)  [right = +]")
        plt.ylabel("y (m)  [forward = +]")
        plt.title("Estimated Drone Path (Navdata-based Dead Reckoning)")
        ...
        # Optional: yaw over time (comment out if you don’t care)
        # plt.figure()
        # plt.plot(np.rad2deg(np.array(path_theta)))
        # plt.xlabel("Sample index")
        # plt.ylabel("Yaw (deg)")
        # plt.title("Estimated Yaw Over Time")
        # plt.grid(True)
        # plt.show()
